...
const EditModal:React.FC<EditModalProps> = ({currentUser}) => {
  const { mutate: mutateFetchedUser } = getUser(currentUser?.id);
  ...
  const onSubmit = useCallback(async () => {
    try {
      ...
      await axios.patch('/api/edit', { userId ,name, username, bio, profileImage, coverImage });
      mutateFetchedUser();
